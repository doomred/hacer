...
var htmlHead, htmlBody, owCSS, hboxCSS, nightCSS, dayCSS, frameNightCSS, frameDayCSS;
dayCSS = '';
  /* last modified: 0.0.6 */
...
function frameOnloadHandler(e) {
  ...
  if(mainFrameList[mainFrameKey] < quoteSize) {
    if(!betterquote(windowCaller)) {
      ...
      setTimeout(function() {callResize(window);}, 500);  /* dirty hack, reduce error */
    }
  } else {
    ...
    for(i = 0; i < fontBar.length; i++) {
      if(fontBar[i].color === '#789922') {
        ...
        fontBar[i].onclick = function () {window.open('http://h.acfun.tv/homepage/ref?tid=' + tmpBuffer, '_blank');};
      }
    }
    styletheframe(windowCaller);
    tmpWindow = windowCaller.parent;
