...
/* listen event and resize the height, failed
window.addEventListener( "message", function(event){
    document.getElementById('resizeme').style.height = event.data+'px';
...
function resizeframe() {
  alert(i);
  document.getElementById('resizeme').height = (heightNew) + "px";
  document.getElementById('resizeme').width = (widthNew) + "px";
...
frameList[frameList.length - 1].onload = window.setTimeout(diveintoframes, 5000); /* the delay num is MAGIC */
...
/* Post about iframe content height, failed 
  frameList[i].document.body.onload = function(){
    ...
    frameList[i].id = 'resizeme';

  }
 */
/* listen for iframe content modification and Post, failed 
  frameList[i].document.body.addEventListener("DOMSubtreeModified", function(){
    top.postMessage(document.body.scrollHeight, '*');
 ...
 */
  ...
  var heightNew = frameList[i].body.scrollHeight;
  var widthNew = frameList[i].document.body.scrollWidth;
  alert(heightNew);alert(widthNew);
  frameList[i].id = 'resizeme';
//  resizeframe();
  ...
  iframeCSS.innerHTML += '.threadpost {padding: 0; margin: 0;}';
